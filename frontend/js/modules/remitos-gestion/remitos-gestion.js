...
function formatRemitoNumero(sequence) {
    if (!Number.isFinite(sequence) || sequence <= 0) {
        return '';
    }

    return `REM-${String(Math.floor(sequence)).padStart(4, '0')}`;
}

function collectNumeroRemitoSequences(remitos) {
    ...
    return remitos
        .map(remito => {
            ...
            const numero = pickValue(remito, ['numeroRemito', 'NumeroRemito', 'numero_remito', 'remitoNumero']);
            return parseRemitoNumericSuffix(numero);
        })
...
async function ensureNumeroRemitoAsignado({ reRender = false } = {}) {
    if (state.formMode !== 'create') {
        return;
    }

    if (sanitizeString(state.formData.numeroRemito) || state.isGeneratingNumeroRemito) {
        return;
    }

    state.isGeneratingNumeroRemito = true;
    ...
    try {
        const { numero, error } = await determineNextNumeroRemito();
        if (state.formMode !== 'create') {
            return;
        }

        if (numero) {
            state.formData.numeroRemito = numero;
        }

        if (error) {
            const message = sanitizeString(error?.message)
                || 'No se pudo obtener el número de remito automáticamente. Se asignará al guardar.';
            state.numeroRemitoError = message;
        } else if (!numero) {
            state.numeroRemitoError = 'No se pudo generar el número de remito automáticamente. Se asignará al guardar.';
        }
    } catch (error) {
        if (state.formMode === 'create') {
            const message = sanitizeString(error?.message)
                || 'No se pudo generar el número de remito automáticamente. Se asignará al guardar.';
            state.numeroRemitoError = message;
        }
    } finally {
...
const state = {
    ...
    clientesError: null,
    selectedClienteKey: '',
    isGeneratingNumeroRemito: false,
    numeroRemitoError: null,
};
...
function renderManagementView() {
    ...
    const hasNumeroRemito = sanitizeString(state.formData.numeroRemito);
    ...
    const numeroRemitoPlaceholderAttr = shouldShowNumeroRemitoPlaceholder
        ...
        : '';
    let numeroRemitoHelpHtml = '';
    if (state.formMode === 'create') {
        if (state.isGeneratingNumeroRemito) {
            numeroRemitoHelpHtml = '<p class="mt-1 text-xs text-gray-500">Generando número automáticamente...</p>';
        } else if (state.numeroRemitoError) {
            numeroRemitoHelpHtml = `<p class="mt-1 text-xs text-red-600">${escapeHtml(state.numeroRemitoError)}</p>`;
        } else if (hasNumeroRemito) {
            numeroRemitoHelpHtml = '<p class="mt-1 text-xs text-gray-500">Número asignado automáticamente.</p>';
        } else {
            numeroRemitoHelpHtml = '<p class="mt-1 text-xs text-gray-500">Se asignará automáticamente al guardar.</p>';
        }
    }
...
function handleAction(action) {
    ...
    if (action === 'open-create') {
        ...
        renderManagementView();
        void ensureClientesLoaded({ reRender: true });
        void ensureNumeroRemitoAsignado({ reRender: true });
        return;
    }
